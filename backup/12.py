...
class ScrabbleGame:
    ...
    def return_tiles_to_rack(self):
        """Cofa wszystkie nowo położone litery na stojak gracza."""
    ...
    def write_to_reports(self, words_info, math_info):
        ...
        with open(self.report_walak_file, "a", encoding="utf-8") as f:
            ...
            f.write(f"[{time_str}] {self.player_names[self.current_player]} zdobywa punkty za: {math_info}\n")
    ...
    def draw(self):
        ...
        if self.exchange_mode:
            ex_info = self.font_small.render("Naciśnij litery do wymiany i ponownie WYMIANA lub ESC", True, (255,100,100))
    ...
    def handle_click(self, pos):
        mx, my = pos
        self.exit_clicks = 0 # Reset licznika wyjścia przy jakiejkolwiek aktywności
        
        if self.game_state == "START_SCREEN":
            if self.btn_start.collidepoint(mx, my): self.game_state = "PLAYING"
            if my < self.board_y + 300: # uproszczone przełączanie kliknięciem
                self.input_active = 1 if my < self.screen.get_height()//2 else 2
            return
        ...
        if self.btn_exit.collidepoint(mx, my): self.handle_exit_btn(); return
        ...
        if 0 <= r < self.board_dim and 0 <= c < self.board_dim:
            ...
            # Obsługa podwójnego kliknięcia na nową literę
    ...
    def handle_exchange(self):
        if not self.exchange_mode:
            self.return_tiles_to_rack() # Cofa literki przy rozpoczęciu wymiany
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if self.game_state == "START_SCREEN":
                        if event.key == pygame.K_BACKSPACE:
                            ...
                            self.input_active = 2 if self.input_active == 1 else 1
                        elif event.key == pygame.K_RETURN:
                            self.game_state = "PLAYING"
                        else:
                            if len(self.player_names[self.input_active]) < 15:
