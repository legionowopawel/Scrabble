...
class ScrabbleGame:
    def __init__(self):
        ...
        pygame.display.set_caption("SCRABBLE - Wersja Poprawiona")
    ...
    def recalculate_dimensions(self):
        ...
        self.font_ui_tiny = pygame.font.SysFont("Verdana", int(sh * 0.015), bold=True)
        ...
        # POZYCJONOWANIE STOJAKÓW: odległość równa szerokości jednej litery od planszy
        ...
        # PRZYCISKI DOLNE (OK, WYMIANA, ZAKOŃCZ, WYJDŹ)
        btn_w = int(sw * 0.18)
        btn_h = int(sh * 0.06)
        spacing = 15
    ...
    def draw(self):
        ...
        # LICZNIK LITER W WORKU
        bag_count_txt = self.font_ui.render(f"Pozostało w worku: {len(self.bag)} liter", True, (255, 255, 0))
        self.screen.blit(bag_count_txt, (sw // 2 - bag_count_txt.get_width() // 2, 5))

        # TURA I PUNKTY (GÓRA)
        turn_txt = self.font_ui.render(f"RUCH: {self.player_names[self.current_player]}", True, (255, 255, 255))
        ...
        for i, l in enumerate(self.racks[1]):
            ...
            self.draw_tile_obj(l, self.rack1_x, self.board_y + i*self.rack_size, col, self.rack_size)
        ...
        for i, l in enumerate(self.racks[2]):
            ...
            self.draw_tile_obj(l, self.rack2_x, self.board_y + i*self.rack_size, col, self.rack_size)

        # PRZYCISKI DOLNE
        ex_color = (0, 100, 200) if self.exchange_mode else (80, 80, 80)
        btns = [
            (self.btn_ok, "ZATWIERDŹ", (0, 100, 0)),
            (self.btn_ex, "WYMIANA", ex_color),
            (self.btn_end, "ZAKOŃCZ I PODSUMUJ", (60, 60, 60)),
            (self.btn_exit, "WYJDŹ", (150, 0, 0))
        ]
        for b, txt, col in btns:
            pygame.draw.rect(self.screen, col, b, border_radius=8)
            pygame.draw.rect(self.screen, (255,255,255), b, 1, border_radius=8)
            f = self.font_ui_tiny if "ZAKOŃCZ" in txt else self.font_ui
            t_s = f.render(txt, True, (255,255,255))
    ...
    def handle_click(self, pos):
        ...
        # KLIKNIĘCIA W STOJAK
        if self.rack1_x <= mx <= self.rack1_x + self.rack_size and self.current_player == 1:
        ...
        if self.rack2_x <= mx <= self.rack2_x + self.rack_size and self.current_player == 2:
            idx = (my - self.board_y) // self.rack_size
    ...
    def handle_exchange(self):
        if len(self.bag) < 7: 
            self.calc_text = "Worek zbyt pusty (wymagane 7)!"; return
        
        if not self.exchange_mode:
            # Wejście w tryb wymiany
            self.return_tiles_to_rack()
            self.exchange_mode = True
            self.exchange_selected = []
            self.calc_text = "WYBIERZ LITERY I KLIKNIJ WYMIANA"
        else:
            # Finalizacja wymiany
            if self.exchange_selected:
                # Usuwamy wybrane litery (od końca, żeby indeksy się nie psuły)
                self.exchange_selected.sort(reverse=True)
                for i in self.exchange_selected:
                    tile = self.racks[self.current_player].pop(i)
                    self.bag.append(tile)
                
                random.shuffle(self.bag)
                # Dobieramy tyle samo liter
                self.racks[self.current_player].extend(self.draw_tiles(len(self.exchange_selected)))
                
                self.current_player = 2 if self.current_player == 1 else 1
                self.play_snd(4)
                self.calc_text = "WYMIENIONO LITER"
            else:
                self.calc_text = "ANULOWANO WYMIANĘ"
    ...
    def handle_exit_logic(self):
        ...
        if now - self.last_exit_click_time < 500: # Szybkie 2-kliknięcie
            pygame.quit(); sys.exit()
        else:
            self.return_tiles_to_rack()
            self.exchange_mode = False
            self.calc_text = "KLIKNIJ 2x WYJDŹ BY ZAMKNĄĆ"
    ...
    def end_game(self, manual=False):
        # Odejmij punkty za pozostałe litery
        for p in [1, 2]:
            minus = sum(LETTERS[l][1] for l in self.racks[p])
            self.scores[p] -= minus
            
        if self.scores[1] > self.scores[2]: self.winner_text = f"WYGRAŁ: {self.player_names[1]}!"
        elif self.scores[2] > self.scores[1]: self.winner_text = f"WYGRAŁ: {self.player_names[2]}!"
