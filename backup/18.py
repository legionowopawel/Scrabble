...
class ScrabbleGame:
    def __init__(self):
        ...
        pygame.display.set_caption("SCRABBLE PRO - Wersja Pełna")
    ...
    def draw_res_list(self, sw, sh):
        ...
        for i, res in enumerate(self.resolutions):
            ...
            pygame.draw.rect(self.screen, (80,80,80), r_rect, 1)
    ...
    def handle_click(self, pos):
        ...
        if self.btn_res_toggle.collidepoint(mx, my):
            self.show_res_menu = True
            return
    ...
    def handle_exchange(self):
        if len(self.bag) < 7: 
            self.calc_text = "Za mało liter w worku do wymiany!"; return
        
        if not self.exchange_mode:
            self.return_tiles_to_rack()
            self.exchange_mode = True
            self.exchange_selected = []
            self.calc_text = "Zaznacz litery na stojaku i kliknij WYMIANA"
        else:
            if self.exchange_selected:
                ...
                self.play_snd(4)
                self.calc_text = f"Wymieniono {len(self.exchange_selected)} liter"
            else:
    ...
    def handle_exit_logic(self):
        ...
        if now - self.last_exit_click_time < 500:
