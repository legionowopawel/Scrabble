...
# ----------------- BONUSY (przykładowe rozmieszczenie) -----------------
def board_bonuses():
    b = {}
...
class ScrabbleEngine:
    ...
    def _is_main_word_horizontal_for_placed(self):
        placed = self.placed_this_turn()
        if not placed: return None
        rows = set(p[0] for p in placed)
        cols = set(p[1] for p in placed)
        if len(rows) == 1: return True
        if len(cols) == 1: return False
        return None

    def words_created_by_placing(self, r, c):
        words = []
        main_horiz = self._is_main_word_horizontal_for_placed()
        if main_horiz is None:
            ...
            return words
        if main_horiz:
            w_main = self.extract_word_from(r, c, horiz=True)
            if w_main: words.append(w_main)
            # vertical cross
            up = (r-1, c); down = (r+1, c)
            ...
            w_main = self.extract_word_from(r, c, horiz=False)
            if w_main: words.append(w_main)
    ...
    def validate_word_online(self, word):
        ...
        try:
            ...
            return True  # fail-safe if no network
    ...
    def commit_move(self):
        ...
        if not ok:
            return False, msg
        word = self.extract_full_placed_word()
        if not word:
        ...
        print(f"Gracz: {word} -> {self.last_player_def[:400]}")
        self.refill_rack(self.player_rack)
        self.turn = "AI"
        return True, f"Słowo '{word}' zaakceptowane. +{pts} pkt."

    # AI routines: try valid single-tile placements, or two-tile center if board empty
                    ...
                    for dr,dc in [(0,1),(1,0),(0,-1),(-1,0)]:
                        ...
                        if 0<=nr<15 and 0<=nc<15 and not self.grid[nr][nc]:
                            for i, let in enumerate(list(self.ai_rack)):
                                ...
                                valid_all = True
                                for w in words:
                                    if len(w) < 2 or not self.validate_word_online(w):
                                        valid_all = False
                                        break
                                if valid_all and words:
                                    ...
                                    print(f"Komputer: {self.last_ai_word} -> {self.last_ai_def[:400]}")
                                    return True
...
class ScrabbleApp:
    ...
    def update_fonts(self):
        ...
        # bonus fonts reduced (approx half)
        self.font_bonus_big = pygame.font.SysFont('Arial', max(10, h//90), bold=True)
        self.font_bonus_small = pygame.font.SysFont('Arial', max(8, h//120), bold=True)
    ...
    def draw_board(self):
        ...
        # show only words (no definitions)
        ...
        self.pass_btn = pygame.Rect(ui_x+150, 220, 140, 30)  # smaller gray button
